...
public final class HashMap<K, V> extends AbstractMap<K, V, HashMap<K, V>> implements Serializable {
    ...
    public static <T, K, V> HashMap<K, V> ofAll(java.util.stream.Stream<? extends T> stream,
        ...
        return HashMap.<K, V>empty().<T>ofStream(stream, keyMapper, valueMapper);
    }
    ...
    public static <T, K, V> HashMap<K, V> ofAll(java.util.stream.Stream<? extends T> stream,
        ...
        return HashMap.<K, V>empty().ofStream(stream, entryMapper);
    }
    ...
    public <K2, V2> HashMap<K2, V2> bimap(Function<? super K, ? extends K2> keyMapper, Function<? super V, ? extends V2> valueMapper) {
        Objects.requireNonNull(keyMapper, "keyMapper is null");
        Objects.requireNonNull(valueMapper, "valueMapper is null");
        final Iterator<? extends Tuple2<K2, V2>> entries = iterator().map(entry -> Tuple.of(keyMapper.apply(entry._1), valueMapper.apply(entry._2)));
    ...
    public HashMap<K, V> orElse(Iterable<? extends Tuple2<K, V>> other) {
        return isEmpty() ? ofEntriesInstance().apply(other) : this;
    }

    @Override
    public HashMap<K, V> orElse(Supplier<? extends Iterable<? extends Tuple2<K, V>>> supplier) {
        return isEmpty() ? ofEntriesInstance().apply(supplier.get()) : this;
    }
    ...
    @Override
    protected Function<Iterable<? extends Tuple2<K, V>>, HashMap<K, V>> ofEntriesInstance() {
        return HashMap::ofEntries;
    }

    @Override
    protected Supplier<HashMap<K, V>> emptyInstance() {
